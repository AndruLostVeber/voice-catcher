...
def deep_analyze(
    ...
    if participants and len(participants) > 2:
        ...
        system_prompt += (
            f"\n\nВ разговоре {len(participants)} участника: {names}. "
            f"speaker_styles, emotion_timeline и all role-поля используют ИМЕНА участников из транскрипта, а не «Я»/«Собеседник»."
        )
